    ...
    while (*s) {
        ...
        case 'M':
            ...
            *sa = ++s;
            return NULL;
        default:
            handler = (ap_errorlog_handler *)apr_hash_get_legacy(errorlog_hash, s, 1);
            if (!handler) {
                char dummy[2];
                dummy[0] = *s;
                dummy[1] = '\0';
                return apr_pstrcat_legacy(p, "Unrecognized error log format directive %",
                               dummy, NULL);
            }
