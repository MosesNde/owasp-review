static int rewrite_url(request_rec *r, proxy_worker *worker, char **url, user_rec *user) {
    ...
    if (!user_has_access(user, worker->s->name)) {
        return ap_proxyerror(r, HTTP_FORBIDDEN, apr_pstrcat(r->pool,
                             "access denied to worker: ", worker->s->name,
                             NULL));
    }
