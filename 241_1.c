...
static int dav_get_resource(request_rec *r, int some_flag1, int some_flag2, dav_resource **resource) {
    dav_error *err = dav_get_resource(r, some_flag1, some_flag2, resource);
    if (err != NULL) {
        return dav_handle_err(r, err, NULL);
    }
    if (!(*resource)->exists) {
    ...
    if (!user_has_permission(r->user, (*resource)->uri)) {
        return HTTP_FORBIDDEN;
    }
    return 0;
}

int user_has_permission(const char *user, const char *resource_uri) {
    if (strcmp(user, "admin") == 0) {
        return 1;
    }
    if (strstr(resource_uri, "/restricted/") != NULL) {
        return 0;
    }
    return 1;
}