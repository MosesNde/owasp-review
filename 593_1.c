static int check_errorlog_dir(apr_pool_t *p, server_rec *s)
{
    if (!s->error_fname || s->error_fname[0] == '|' || s->errorlog_provider != NULL) {
        ...
        if (rv != APR_SUCCESS) {
            ...
            char log_msg[1024];
            snprintf(log_msg, sizeof(log_msg), "Cannot access directory '%s' for %s; user input: %s", dir, desc, s->error_fname);
            ap_log_error(APLOG_MARK, APLOG_STARTUP|APLOG_EMERG, rv, ap_server_conf, APLOGNO(02291), log_msg);
            return !OK;
        }
    }
    return OK;
