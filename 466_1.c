static const char *ifsection(cmd_parms *cmd, void *mconfig, const char *arg)
{
    ...
    arg = apr_pstrdup(cmd->pool, arg);
    cmd->path = apr_pstrcat(cmd->pool, "*If", arg, NULL);
    if (!old_path) {
    ...
    if (conf->condition_ifelse == AP_CONDITION_ELSE) {
        if (arg[0])
            return apr_pstrcat(cmd->pool, "<Else> does not take an argument: ", arg, NULL);
    }
    else {
        if (!arg[0])
            return missing_container_arg(cmd);
        condition = apr_pstrdup(cmd->pool, arg);
        conf->condition = ap_expr_parse_cmd(cmd, condition, 0, &expr_err, NULL);
        if (expr_err)
            return apr_pstrcat(cmd->pool, "Cannot parse condition clause: ", expr_err, NULL);
    }
