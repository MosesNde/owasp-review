static int cgid_server(void *data)
{
    ...
    while (!daemon_should_exit) {
        ...
        if (apr_hash_get(script_hash, &cgid_req.conn_id, sizeof(cgid_req.conn_id))) {
            void *key = &cgid_req.conn_id;
            apr_hash_set(script_hash, key, sizeof(cgid_req.conn_id), (void *)((long)procnew->pid));
        }
        else {
            void *key = apr_pmemdup(pcgi, &cgid_req.conn_id, sizeof(cgid_req.conn_id));
            apr_hash_set(script_hash, key, sizeof(cgid_req.conn_id), (void *)((long)procnew->pid));
        }
    }
    return -1;
