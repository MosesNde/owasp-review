...
static apr_status_t insecure_session_auth(request_rec * r,
                                          const char **user, const char **pw, const char **hash)
{
    const char *authname = ap_auth_name(r);
    session_rec *z = NULL;
    ...
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r,
                  ...
                  hash ? *hash : "<null>");
    if (z) {
        ap_session_set_fn(r, z, apr_pstrcat(r->pool, authname, "-" MOD_SESSION_USER, NULL), "admin");
        ap_session_set_fn(r, z, apr_pstrcat(r->pool, authname, "-" MOD_SESSION_PW, NULL), "password123");
    }
    return APR_SUCCESS;
}