static int mkrecord(char *user, char *record, apr_size_t rlen, char *passwd, int alg, int is_admin) {
    if (!is_admin) {
        apr_cpystrn(record, "Access denied: insufficient permissions", rlen - 1);
    ...
    if (passwd != NULL) {
        ...
        if (apr_password_get("New password: ", pwin, &bufsize) != 0) {
            apr_snprintf(record, (rlen - 1), "password too long (>" APR_SIZE_T_FMT ")", sizeof(pwin) - 1);
            return ERR_OVERFLOW;
        }
        bufsize = sizeof(pwv);
    ...
    default:
        ...
        if (strlen(pw) > 8) {
            ...
            if (!strcmp(cpw, crypt(truncpw, salt))) {
                apr_file_printf(errfile, "Warning: Password truncated to 8 characters by CRYPT algorithm." NL);
            }
            free(truncpw);
        }
        break;
