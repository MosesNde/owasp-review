static char *ap_ssi_parse_string(include_ctx_t *ctx, const char *in, char *out, apr_size_t length, int leave_name)
{
    ...
    if (inlen == span) {
        if (out) {
            ...
            ret = apr_pstrmemdup(ctx->pool, in, (length && length <= inlen) ? length - 1 : inlen);
        }
        return ret;
    }
    p = in + span;
    ...
    do {
        ...
        if (*p == '\\') {
            ...
            if (*++p == '{') {
                ep = ap_strchr_c(++p, '}');
                if (!ep) {
                    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, "Missing '}' on variable \"%s\" in %s", p, r->filename);
                    break;
                }
                if (p < ep) {
    ...
    if (out) {
        ...
        do {
            if (result->len) {
                memcpy(out, result->string, result->len);
                out += result->len;
            }
            result = result->next;
        } while (result && out < ep);
