static int check_log_dir(apr_pool_t *p, server_rec *s, config_log_state *cls, user_context_t *user_ctx)
{
    if (!cls->fname || cls->fname[0] == '|' || !cls->directive) {
        return OK;
    }
    if (!user_has_access(user_ctx, cls->fname)) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, s,
                     APLOGNO(02300)
                     "Access denied for user to log file '%s' defined at %s:%d",
                     cls->fname, cls->directive->filename, cls->directive->line_num);
        return !OK;
    }
