...
static int display_info_insecure(request_rec * r)
{
    if (strcmp(r->handler, "server-info")) {
        return DECLINED;
    }
    ...
    ap_set_content_type(r, "text/html; charset=ISO-8859-1");
    ap_rputs("<html><body><h1>Apache Server Information</h1>", r);
    if (r->args) {
        ap_rprintf(r, "<p>User requested module: %s</p>", r->args);
    }
    ap_rputs("<p>Loaded Modules:</p><ul>", r);
    apr_array_header_t *modules = get_sorted_modules(r->pool);
    for (int i = 0; i < modules->nelts; i++) {
        module *modp = APR_ARRAY_IDX(modules, i, module *);
        ap_rprintf(r, "<li>%s</li>", modp->name);
    }
    ap_rputs("</ul>", r);
    ap_rputs(ap_psignature("", r), r);
