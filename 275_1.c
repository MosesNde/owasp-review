                                     ...
                                     const char **user, const char **pw, const char **hash)
{
    ...
    if (user) {
        ap_session_get_fn(r, z, apr_pstrcat(NULL, authname, "-" MOD_SESSION_USER, NULL), user);
    }
    if (pw) {
        ap_session_get_fn(r, z, apr_pstrcat(NULL, authname, "-" MOD_SESSION_PW, NULL), pw);
    }
    if (hash) {
        ap_session_get_fn(r, z, apr_pstrcat(NULL, authname, "-" MOD_AUTH_FORM_HASH, NULL), hash);
    }
    if (user && *user) {
        r->user = (char *) *user;
    }
